...
const crearDetalle = async (req, res) => {
  ...
  try {
    const { id_venta, id_producto, cantidad, precio_unitario, descuento_linea } = req.body;
    ...
    if (['Anulado','Pagado'].includes(venta.rows[0].estado))
      return res.status(400).json({ message: `Venta en estado "${venta.rows[0].estado}"` });
...
const actualizarDetalle = async (req, res) => {
  ...
  try {
    ...
    if (['Anulado','Pagado'].includes(check.rows[0].estado))
      return res.status(400).json({ message: `Venta en estado "${check.rows[0].estado}"` });
    ...
    if (diff > 0) {
      ...
      if ((stockRes.rows[0]?.stock ?? 0) < diff)
        return res.status(400).json({ message: `Stock insuficiente. Disponible extra: ${stockRes.rows[0]?.stock ?? 0}` });
    }
...
const eliminarDetalle = async (req, res) => {
  ...
  try {
    ...
    await client.query('BEGIN');
    await client.query(`DELETE FROM "DetalleVenta" WHERE id_detalle_venta=$1`, [req.params.id]);
    await client.query(`UPDATE "Productos" SET stock=stock+$1 WHERE id_producto=$2`, [cantidad, id_producto]);
    await recalcular(client, id_venta);
    await client.query('COMMIT');
